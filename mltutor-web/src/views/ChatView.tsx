...
import type { Message, AppSettings, ChatHistoryItem, ChatRequest } from '../types';
...
const ChatView: React.FC<ChatViewProps> = ({ messages, settings, updateMessages, pendingQuestion, onConsumePending }) => {
...
const renderMarkdown = useCallback((text: string) => {
    ...
    processed = processed.replace(/\\\[(.+?)\\\]/gs, (_, content) => {
      ...
      tokens.push({ key, html: String.raw`\[${content}\]` });
      return key;
    });
    processed = processed.replace(/\$\$(.+?)\$\$/gs, (_, content) => {
      const key = `@@BLOCK_${tokens.length}@@`;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-280px)] min-h-[600px] bg-white rounded-[2rem] border border-slate-200/80 shadow-xl shadow-slate-200/50 overflow-hidden relative">
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth bg-slate-50/30">
        {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center px-4">
                ...
                <p className="text-slate-500 max-w-xs leading-relaxed">
                  随时询问关于上传教材的任何问题。我可以解释概念、总结章节或解答您的疑惑。
                </p>
                <div className="grid grid-cols-1 gap-2 mt-8 w-full max-w-md">
                  {["机器学习有哪些常见模型？", "深度学习和传统 ML 有何区别？", "如何提升模型泛化能力？"].map(q => (
                    <button 
                      ...
                      className="px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-600 hover:border-indigo-300 hover:text-indigo-600 hover:shadow-sm transition-all text-left"
                    >
                      "{q}"
                    </button>
                  ))}
        ...
        {messages.map((msg) => (
          ...
          >
            <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-sm border ${msg.role === 'user' ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-100 text-indigo-600'}`}>
                ...
                {msg.role === 'assistant' && (
                    <div className="mt-3 flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-1 ml-1">
                            ...
                            <button className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-red-600 transition-colors" title="无帮助"><ThumbsDown size={14}/></button>
                        </div>
                    </div>
                )}
                {msg.role === 'user' && (
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white to-transparent pt-10">
        <form onSubmit={handleSend} className="relative flex items-center max-w-4xl mx-auto">
            <input 
                ...
                className="w-full pl-6 pr-16 py-4 bg-white border border-slate-200 text-slate-900 rounded-2xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-lg shadow-slate-200/40 text-[15px]"
            />
            <button 
                type="submit" 
                disabled={!input.trim() || isSending}
                className="absolute right-3 p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 disabled:hover:bg-indigo-600 transition-all shadow-md shadow-indigo-200"
            >
                <Send size={18} strokeWidth={2.5} />
            </button>
        </form>
        {error && (
          <div className="max-w-4xl mx-auto mt-3 text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-2">
            {error}
          </div>
        )}
      </div>
